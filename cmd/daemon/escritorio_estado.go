...
func (e *escritorio) abrirGrafo(raiz string) {
	// El token se toma AQUÍ, en el orden real de los pedidos: la goroutine de
	// abajo puede ganar el candado en cualquier orden, pero solo la del último
	// pedido sigue vigente al publicar.
	gen := e.grafoGen.Add(1)
	c := e.contextoDelGrafo(raiz)
	go func() {
		if e.publicarGrafoSiVigente(gen, c) {
			application.InvokeAsync(e.abrirVentanaExplorador)
		}
	}()
}

// publicarGrafoSiVigente construye y publica el grafo SOLO si el pedido sigue
// siendo el último. Store y apertura van bajo el mismo candado (el par es
// indivisible); el ORDEN lo da el token — un pedido viejo que gane el candado
// tarde descubre que otra generación tomó el relevo y no publica nada, jamás
// pisa a la más nueva ([17] del plan; mismo patrón que la bandeja, tray.go).
func (e *escritorio) publicarGrafoSiVigente(gen uint64, c contextoGrafo) bool {
	e.grafoMu.Lock()
	defer e.grafoMu.Unlock()
	if e.grafoGen.Load() != gen {
		log.Printf("grafo de %s: pedido superado por otro más nuevo — no se publica", c.nombre)
		return false
	}
	e.prepararGrafo(c)
	return true
