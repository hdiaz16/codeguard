...
func TestElOrbeDistingueLaDecisionDelEquipoDeLaAveria(t *testing.T) {
	...
	// Y la config ilegible ya ni siquiera llega como "skipped": el productor la
	// declara failed:config, que aquí es un naranja de avería sin matices.
	if got := orbStateFor(&panelPayload{Verdict: "skipped", Outcome: "failed"}); got != "degraded" {
		t.Errorf("un análisis FALLIDO pintó %q: la cobertura tiene un agujero y hay que verlo", got)
	}
}
...
func TestElOrbeNoTieneCriterioPropioSobreQuienDecidioElSalto(t *testing.T) {
	...
	for _, m := range motivos {
		...
		p := &panelPayload{Verdict: "skipped", Outcome: "skipped", Reason: m}
