...
func TestInitConLaBanderaLlegaHastaLaEscritura(t *testing.T) {
	...
	// Desde que init verifica sus POSTCONDICIONES antes del LISTO ([31]),
	// el fixture debe aprovisionarlas como haría una máquina real: rulepack
	// resoluble y binario donde binpath apunta.
...
// aprovisionarPostcondicionesDeInit deja el entorno del test como una máquina
// bien instalada: el rulepack vendoreado en el repo (resoluble) y un
// codeguard.exe junto al ejecutable del test (adonde install apunta binpath).
func aprovisionarPostcondicionesDeInit(t *testing.T, repo string) {
	t.Helper()
	...
	bin := filepath.Join(filepath.Dir(exe), "codeguard.exe")
	if _, err := os.Stat(bin); os.IsNotExist(err) {
		if err := os.WriteFile(bin, []byte("stub de test"), 0o755); err != nil {
			t.Skipf("no se pudo aprovisionar el binario del fixture: %v", err)
		}
	}
}
