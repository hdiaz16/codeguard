...
func TestNoDiceCompletadoSiUnaCapaNoCorrio(t *testing.T) {
	...
	informe := encabezado(construirInforme(cfg, res, pipeline.Finalizar(res, "", nil).GarantiaRota, nil, nil, nil, nil, nil, false, false, ""))
...
func TestConBloqueantesTambienAvisaDeLoQueNoCorrio(t *testing.T) {
	...
	informe := encabezado(construirInforme(cfg, res, pipeline.Finalizar(res, "", nil).GarantiaRota, bloq, nil, nil, nil, nil, false, false, ""))
