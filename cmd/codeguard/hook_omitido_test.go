...
func TestUnCommitNormalConservaSuVeredicto(t *testing.T) {
	...
	//
	// Este camino va sin daemon, y ANTES eso pintaba PARCIAL: el criterio del
	// hook era len(Degraded)>0 crudo y daemon:offline cuenta. Pero en local el
	// análisis corre ENTERO (garantia.go la exime por eso), así que el PARCIAL
	// prometía una revisión a medias que no ocurrió — y el CI decía OK del
	// mismo Degraded. Desde el AnalysisOutcome (consejo, turnos 61-68) la
	// frontera de PARCIAL es la garantía rota, la misma en toda superficie: el
	// veredicto legítimo aquí es el ✓, con el aviso de la corrida en frío como
	// orientación aparte.
	if strings.Contains(salida, "— PARCIAL") {
		t.Errorf("daemon:offline volvió a pintar PARCIAL: el análisis local corre entero "+
			"y este veredicto prometía media revisión donde hubo una completa:\n%s", salida)
	}
	if !strings.Contains(salida, "listo — commit permitido") {
		t.Errorf("el hook no afirmó su veredicto positivo:\n%s", salida)
	}
	if !strings.Contains(salida, "el agente no estaba corriendo") {
		t.Errorf("la corrida en frío perdió su aviso de orientación:\n%s", salida)
	}
