...
// chequeosDelRepo construye los chequeos del repo SIN imprimirlos: los
// consumen status (texto) y doctor (texto y --json) — UNA lista de verdades,
// dos renders. Antes la verdad y el print vivian pegados y doctor habria
...
func chequeosDelRepo(root string) ([]string, map[string]chequeo) {
	...
	if cfg != nil {
		id, err := rulepack.Resolver(root, cfg.Rulepack)
		switch {
		case err == nil:
			donde := "junto al binario"
			if id.Source == rulepack.SourceVendored {
				donde = "vendoreado en el repo"
			}
			checks["rulepack"] = chequeo{true, cfg.Rulepack + " (" + donde + ", digest " + corto(id.Digest) + ")"}
		case errors.Is(err, rulepack.ErrNoEncontrado):
			checks["rulepack"] = chequeo{false,
				"no encuentro el rulepack " + cfg.Rulepack + " → sin paridad con el CI; reinstala o vendoréalo"}
		default:
			checks["rulepack"] = chequeo{false, "presente pero sin identidad verificable: " + err.Error()}
		}
	}
	...
	// legítimamente en un repo limpio — y el remedio propuesto no cambiaba
	// nada: un rojo sin salida), y el archivo ILEGIBLE (permisos, disco:
	// el caso grave, con el mismo texto rutinario). Medido en la bitácora
	// 2026-08-22 como una de las mentiras del veredicto.
	if cfg != nil {
		if supresiones, err := baseline.Load(root); err != nil {
			checks["baseline"] = chequeo{false,
				"la baseline existe y NO SE PUEDE LEER (" + unaSolaLinea(err.Error()) +
					") → nada se suprime: toda la deuda aceptada volverá a bloquear"}
		} else if n := len(supresiones); n > 0 {
			checks["baseline"] = chequeo{true, fmt.Sprintf("%d hallazgos preexistentes suprimidos", n)}
		} else if _, statErr := os.Stat(filepath.Join(root, filepath.FromSlash(baseline.RelPath))); statErr == nil {
			// Presente y vacía: el estado LEGÍTIMO de un repo sin deuda. Un ✗
			// aquí mandaba a correr un comando que regenera el mismo archivo.
			checks["baseline"] = chequeo{true, "baseline presente, sin deuda que suprimir (repo limpio)"}
		} else {
...
// valor completo estampado en el veredicto y en runs.
