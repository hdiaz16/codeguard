...
- **2026-08-12 (noche)** — **La auditoría de nuestra propia cadena no miraba nada y firmaba que estaba limpia.** Usaba `trivy fs`, que busca manifiestos de código fuente; lo que repartimos son artefactos instalados y eso pide `rootfs`. Daba "0 objetivos, 0 vulnerabilidades" en los cinco artefactos y el CI usaba ese paso como compuerta. Con `rootfs`, los mismos archivos sin tocar: **88 hallazgos**. Salió metiéndole un `log4j-core 2.14.1` y viendo que no encontraba Log4Shell — ese control se queda como prueba, porque un escáner ciego y uno sano dan la misma salida. Ahora "cero objetivos" es un error, no un limpio. Arreglado de verdad: trivy 0.71→0.73 (30→4 hallazgos), govulncheck y staticcheck a cero recompilándolos con go1.26.5 (heredaban stdlib de un Go viejo), y `cryptography` fuera porque **no es nuestra** — se escaneaba el site-packages entero y nos acusaba de paquetes ajenos; ahora la clausura sale de `importlib.metadata`. De paso, grieta de paridad cerrada: el CI compilaba gitleaks mientras las máquinas descargaban el zip oficial — misma etiqueta, bytes distintos, en el único motor fail-closed. Los 30 altos restantes no se arreglan actualizando (ya estamos en la última de cada autor) y viven en `excepciones.json` **sin firmar**: hay que nombrar el riesgo, justificarlo para nuestra invocación, firmarlo y ponerle caducidad. Sin firma no aplican y `engines --auditar` sale con 1 — **el paso de auditoría del CI está rojo a propósito hasta que el usuario firme**.
