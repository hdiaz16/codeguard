...
// Validar comprueba que unos bytes de config parsean y casan con el esquema,
// SIN tocar el disco. Existe para que init valide el YAML que COMPONE antes
// de escribirlo (turno 92): hasta entonces el bloque org-llm.yaml se
// inyectaba crudo y el primer diagnóstico del typo salía disfrazado de
// «falló el baseline» tres pasos después.
func Validar(raw []byte) error {
	k := koanf.New(".")
	if err := k.Load(rawbytes.Provider(raw), yaml.Parser()); err != nil {
		return fmt.Errorf("config.yaml inválido: %w", err)
	}
	var cfg Config
	if err := k.Unmarshal("", &cfg); err != nil {
		return fmt.Errorf("config.yaml no coincide con el esquema: %w", err)
	}
	return nil
}
