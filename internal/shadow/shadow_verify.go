...
func verify(req *ipc.Request, pillar finding.Pillar, content string, deterministic []finding.Finding) ([]finding.Finding, int) {
	...
	for _, lf := range parsed.Findings {
		...
		lineas := fileLines(req.RepoRoot, file)
		switch {
		case !inDiff[strings.ToLower(file)]: // ¿el archivo citado está en el diff?
			rejected++
			continue
		case lf.Line < 1 || lf.Line > len(lineas): // ¿la línea existe?
			rejected++
			continue
		...
		f := finding.Finding{
			...
			Verified: true,
			Source:   finding.LLM,
			// La línea REAL del archivo, no el mensaje del modelo (H202): con
			// el mensaje, dos hallazgos distintos con la misma frase en el
			// mismo archivo colapsaban en una huella — la clase #9 por la
			// puerta del LLM. La línea ya estaba leída para el chequeo de
			// existencia; usarla es gratis. La reconstrucción legacy (v1
			// hasheaba el mensaje) vive en finding.contenidoLegacy.
			LineContent: lineas[lf.Line-1],
		}
		// La huella NO se calcula aquí: la asigna finding.AsignarHuellas en
		// shadow.Run, colectivamente, cuando ya se ven los tres pilares — la
		// regla de ambigüedad no funciona por partes.
		ok = append(ok, f)
	}
	return ok, rejected
}

// fileLines devuelve las líneas del archivo, confinado al repo (gosec G304):
// rutas citadas por el modelo no salen de él. nil si no se puede leer.
func fileLines(repoRoot, rel string) []string {
