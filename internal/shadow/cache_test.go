...
func TestUnPilarCaidoNoCacheaElResultado(t *testing.T) {
	...
	// La clave la compone claveDelCache, la MISMA función que usa el motor: si
	// se rearmara a mano aquí, el día que cambie la clave este test seguiría
	// «pasando» sin mirar la entrada que le toca.
	if _, hit := r.Store.DiffCacheGet(req.RepoID, diffSHA, req.RulepackVersion, req.ConfigHash, claveDelCache(cfg)); hit {
...
func TestConTodosLosPilaresSiSeCachea(t *testing.T) {
	...
	diffSHA := sha256hex(req.DiffUnified)
	crudo, hit := r.Store.DiffCacheGet(req.RepoID, diffSHA, req.RulepackVersion, req.ConfigHash, claveDelCache(cfg))
	if !hit {
		t.Fatal("con los tres pilares respondiendo el resultado es completo y debe cachearse")
	}
	// Y lo cacheado son las respuestas CRUDAS con su versión, no hallazgos ya
	// verificados: el acierto futuro re-verifica, y para eso necesita la
	// materia prima (la confianza no sobrevive en finding.Finding).
	var entrada sombraCacheada
	if err := json.Unmarshal([]byte(crudo), &entrada); err != nil {
		t.Fatalf("la entrada no es sombraCacheada: %v\n%s", err, crudo)
	}
	if entrada.Version != versionVerificador || len(entrada.Crudas) != 3 {
		t.Errorf("la entrada debe llevar la versión del verificador y los TRES pilares crudos: %+v", entrada)
	}
}
