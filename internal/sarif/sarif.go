...
func Write(path, version string, findings []finding.Finding, outcome pipeline.AnalysisOutcome) error {
	...
	for _, f := range findings {
		...
		results = append(results, Result{
			...
			// DOS claves durante la ventana dual de huellas: partialFingerprints
			// gobierna la IDENTIDAD de las alertas en GitHub code-scanning, y
			// meter el valor v2 bajo la clave /v1 habría renacido cada alerta
			// como nueva. SARIF admite varias claves a la vez: /v2 lleva la
			// primaria y /v1 el alias exacto — las alertas viejas siguen
			// casando por /v1 mientras la ventana viva.
			PartialFingerprints: huellasSARIF(f),
		})
	}
...
// huellasSARIF compone las claves de identidad de la alerta: la primaria bajo
// /v2 y el alias v1 bajo la clave histórica /v1, mientras dure la ventana
// dual. Un hallazgo sin alias (p. ej. futuro, post-ventana) emite solo /v2.
func huellasSARIF(f finding.Finding) map[string]string {
	claves := map[string]string{"codeguardFingerprint/v2": f.Fingerprint}
	if f.LegacyFingerprint != "" {
		claves["codeguardFingerprint/v1"] = f.LegacyFingerprint
	}
	return claves
}
