...
func TestElCacheInvalidaEnSusCuatroEjes(t *testing.T) {
	...
	t.Run("version del rulepack", func(t *testing.T) {
		...
		//
		// SALVO por identidad, lo del propio config.yaml: este subtest lo EDITA
		// como vehículo del eje, y la huella v2 lleva el ancla contextual — un
		// hallazgo cuya línea vecina es justo la que el test cambia vuelve con
		// identidad NUEVA, por diseño (turnos 83-84: editar contexto puede
		// renovar identidades; jamás entierra código nuevo). Su regreso se
		// exige aparte, por presencia en el archivo.
		nuevas := analizarHuellas(t, bin, repo, datos)
		exigirQueVuelvan(t, sinArchivo(reales, ".codeguard/config.yaml"), nuevas,
			"al cambiar el RULEPACK", "actualizar las reglas sin que cambie el veredicto es una actualización que no llega")
		if len(deArchivo(reales, ".codeguard/config.yaml")) > 0 && len(deArchivo(nuevas, ".codeguard/config.yaml")) == 0 {
			t.Error("el hallazgo del propio config.yaml no volvió con NINGUNA identidad: eso ya no es el ancla, es el caché sirviendo lo viejo")
		}
	...
	t.Run("config del repo", func(t *testing.T) {
		...
		// El propio config.yaml queda fuera de la comparación por identidad,
		// por lo mismo que en el eje del rulepack: el test lo edita y el ancla
		// contextual de un hallazgo vecino cambia con él, por diseño.
		nuevas := analizarHuellas(t, bin, repo, datos)
		exigirQueVuelvan(t, sinArchivo(reales, ".codeguard/config.yaml"), nuevas,
			"al cambiar la CONFIG del repo", "el veredicto depende de la configuración y el caché la ignoraría")
		if len(deArchivo(reales, ".codeguard/config.yaml")) > 0 && len(deArchivo(nuevas, ".codeguard/config.yaml")) == 0 {
			t.Error("el hallazgo del propio config.yaml no volvió con NINGUNA identidad")
		}
...
// sinArchivo descarta los hallazgos de una ruta: el complemento de deArchivo,
// para cuando la comparación por identidad no aplica a un archivo que el
// propio test edita (el ancla de la huella v2 cambia con las vecinas).
func sinArchivo(hs map[string]hallazgo, ruta string) map[string]hallazgo {
	out := map[string]hallazgo{}
	for k, v := range hs {
		if v.archivo != ruta {
			out[k] = v
...
func copiarRulepackComo(t *testing.T, repo, version string) {
	t.Helper()
	// El ORIGEN se descubre en el árbol (la versión más nueva presente) en vez
	// de cablear un número que envejece con cada bump: al pasar de 2026.08.2 a
	// .3 este helper quedó apuntando a un directorio inexistente y toda prueba
	// que vendorea rulepack se saltaba EN SILENCIO. El destino sí conserva el
	// nombre que pinnea el repo de juguete: es su promesa, no la nuestra.
	origen := rulepackDelArbol(t)
...
// rulepackDelArbol resuelve el semgrep/ de la versión más nueva vendoreada en
// ESTE repo — la fuente de verdad de las reglas para los repos de juguete.
func rulepackDelArbol(t *testing.T) string {
	t.Helper()
	base := filepath.Join(raizDelRepo(t), "rulepacks")
	entradas, err := os.ReadDir(base)
	if err != nil {
		t.Skipf("no encuentro rulepacks/ en el árbol: %v", err)
	}
	...
	if len(versiones) == 0 {
		t.Skip("rulepacks/ sin versiones en el árbol")
	}
	sort.Strings(versiones)
	return filepath.Join(base, versiones[len(versiones)-1], "semgrep")
}
