...
func TestLaBaselineNoPuedeTaparUnSecreto(t *testing.T) {
	...
	// con --si, porque el prompt es fail-closed desde 2026-08-23 y sin
	// consentimiento explícito la baseline ya ni se escribe (lo que esta
	// prueba mide es que ESCRIBIÉNDOLA el secreto sigue bloqueando).
	salida, _ = correr(t, bin, repo, "baseline", "--si")
...
func TestLaBaselineSuprimeSoloLoPreexistente(t *testing.T) {
	...
	// --si explícito: el prompt es fail-closed desde 2026-08-23 (un error de
	// lectura por pipe/EOF ya no se interpreta como consentimiento — este test
	// escribía la baseline GRACIAS a ese fail-open y lo certificaba de gratis).
	salida, _ := correr(t, bin, repo, "baseline", "--si")
