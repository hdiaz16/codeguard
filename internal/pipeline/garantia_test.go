...
func TestQueDegradacionesRompenLaGarantia(t *testing.T) {
	casos := []struct {
		...
		{"daemon incompatible es despliegue mixto: el análisis corrió entero en local",
			[]string{"daemon:incompatible"}, false},
...
func TestLasDeliberadasGananALosSufijosGenericos(t *testing.T) {
	for _, d := range []string{"deterministic:diff_too_large", "daemon:offline", "daemon:incompatible", "squawk:migracion-sin-vigilar"} {
