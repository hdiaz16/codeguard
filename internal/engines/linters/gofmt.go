...
func (e GoFmt) Run(ctx context.Context, in engines.Input) ([]finding.Finding, error) {
	...
	if e.Cache != nil {
		e.Cache.Guardar(porArchivoGoFmt(nuevos, verificados, in.RepoRoot))
	}
...
func porArchivoGoFmt(fs []finding.Finding, archivos []gitdiff.ChangedFile, repoRoot string) []engines.Cacheable {
	porRuta := map[string][]finding.Finding{}
	for _, f := range fs {
		porRuta[f.File] = append(porRuta[f.File], f)
	}
	// La clave no lleva ruta: dos archivos de contenido idéntico la comparten,
	// así que la vigencia tiene que cubrir TODAS las rutas que la produjeron.
	porClave := map[string][]finding.Finding{}
	relsPorClave := map[string][]string{}
	shaPorClave := map[string]string{}
	var orden []string
	for _, a := range archivos {
		if a.SHA256 == "" {
			continue
		}
		clave := "gofmt:" + a.SHA256
		if _, ya := porClave[clave]; !ya {
			orden = append(orden, clave)
			porClave[clave] = porRuta[a.Path]
			shaPorClave[clave] = a.SHA256
		}
		relsPorClave[clave] = append(relsPorClave[clave], a.Path)
	}
