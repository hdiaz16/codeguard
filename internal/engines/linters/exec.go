...
// perfilPorBin es LA DECLARACIÓN de entorno de los motores que corren por el
// runner genérico (W4, t.116): cada binario recibe su familia de variables y
// nada más. Lo NO declarado corre con el puro piso — fail-closed: el motor
// nuevo que necesite su toolchain lo declara aquí (y su fixture de perfil en
// proc/perfiles lo mide) en vez de heredar el entorno de todos.
func perfilPorBin(bin string) proc.Perfil {
	nombre := strings.ToLower(filepath.Base(bin))
	for _, ext := range []string{".exe", ".cmd", ".bat"} {
		nombre = strings.TrimSuffix(nombre, ext)
	}
	switch nombre {
	case "go", "gosec":
		return proc.PerfilGo // toolchain de Go offline (govet compila; gosec carga el módulo)
	case "ruff", "bandit", "python":
		return proc.PerfilPython
	case "dotnet":
		return proc.PerfilDotnet // dotnet format: offline (el build y vuln tienen sus propios sitios)
	case "npx", "tsc", "node":
		return proc.PerfilNode
	default:
		return proc.PerfilBasico
	}
}
