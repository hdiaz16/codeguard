...
func TestHallazgosESLintMapeaSeveridadesYArreglos(t *testing.T) {
	...
	for _, f := range fs {
		...
		if f.Message == "" { // W6: el parser ya no pone LineContent; la línea la lee AsignarHuellas del archivo. El insumo del parser es Message.
			// La identidad la asigna finding.AsignarHuellas (huellas v2);
			// el parser garantiza el INSUMO del hash.
			t.Errorf("%s: sin Message el parser no dejó insumo para la huella", f.RuleKey)
		}
...
// reescribir la ruta — la identidad (huella v2) la asigna después
// finding.AsignarHuellas sobre esa ruta reescrita, así que servir la ruta
// equivocada atribuiría el hallazgo (y su identidad) al otro archivo.
func TestCacheReescribeLaRutaAlReproducirUnAcierto(t *testing.T) {
	...
	// El acierto ya NO recalcula la huella (murieron las 5 copias del
	// re-cálculo): sirve el hallazgo con la ruta reescrita y la identidad la
	// asigna finding.AsignarHuellas en el pipeline. Lo que este test fija es
	// que la reescritura de ruta produce identidades DISTINTAS por archivo al
	// asignar — la preocupación original, en el contrato nuevo.
	reasignados := []finding.Finding{out[0], {RuleKey: out[0].RuleKey, Engine: "eslint",
		File: "src/original.js", LineContent: out[0].LineContent}}
	finding.AsignarHuellas(reasignados, nil)
	if reasignados[0].Fingerprint == huellaOriginal {
		t.Error("la huella del acierto atribuido a otra ruta no puede ser la de la ruta original")
	}
	if reasignados[0].Fingerprint == reasignados[1].Fingerprint {
		t.Error("dos rutas distintas con el mismo contenido deben tener identidades distintas")
	}
}
...
func TestCacheDeArchivosGuardaTambienLosLimpios(t *testing.T) {
	...
	entradas := cacheDeArchivosJS(fs, analizados, t.TempDir())
	if len(entradas) != 2 {
		t.Fatalf("esperaba 2 claves (la sin huella queda fuera), obtuve %v", entradas)
	}
