	...
	for _, a := range rep.Files {
		...
		for _, v := range a.Violations {
			...
			f := finding.Finding{
				...
				// (W6, t.128). El fingerprint (§9) sigue sin número de línea para
				// sobrevivir a los desplazamientos; lo que ahora distingue dos
				// violaciones de la misma regla en el mismo archivo es el ANCLA
				// (las vecinas de cada línea), no la descripción metida a mano. La
				// v1 hasheaba «regla descripción» y su alias se reconstruye en
				// contenidoLegacy desde RuleKey+Message.
			}
