...
func TestHallazgosPMDMapeaPrioridadesDesdeLaCapturaReal(t *testing.T) {
	...
	for _, f := range fs {
		...
		if f.Message == "" {
			// W6 (t.128): pmd es híbrido; el parser ya no pone LineContent (la
			// línea real la lee AsignarHuellas). El insumo que el parser
			// garantiza para la huella —y para reconstruir la legacy v1— es el
			// Message (la descripción de la violación).
