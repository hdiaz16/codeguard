...
func TestReporteConSecretoDaHallazgoBloqueante(t *testing.T) {
	...
	// La identidad ya no la asigna el parser: la asigna finding.AsignarHuellas
	// (colectiva, huellas v2). Lo que el parser SÍ debe garantizar es el
	// INSUMO — un LineContent con el que la huella distinga este secreto.
	if h.LineContent == "" {
		t.Error("sin LineContent la huella saldría sobre vacío: el mismo secreto no se reconocería entre corridas")
	}
	finding.AsignarHuellas(hallazgos, nil)
	if v, ok := finding.ParseHuella(hallazgos[0].Fingerprint); !ok || v != 2 {
		t.Errorf("el secreto no recibió una huella v2 válida: %q", hallazgos[0].Fingerprint)
	}
}
