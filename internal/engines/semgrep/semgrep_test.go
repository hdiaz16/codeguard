...
// Payload real de portal-cliente: un archivo TS parcialmente parseado. Declarar
...
// en portal-cliente un archivo parcialmente parseado convivia con 45 hallazgos
// validos.
...
// La exigencia de la revisión, ahora como RECIBO y no como aviso (W6 Q2): un
// PartialParsing con cero resultados NO puede presentarse como limpio. Se
// declara como cobertura PARCIAL de su objetivo; el orquestador degrada esa
// capa —confirmado en pipeline.TestUnReciboParcialTambienDegrada— SIN tirar los
// hallazgos válidos que conviven con él (portal-cliente: 45). Aquí se fija que
// semgrep mapea el objetivo parcial a un recibo parcial y normaliza su ruta.
func TestUnParcialSeDeclaraComoReciboParcial(t *testing.T) {
	res := parsear(t, `{"results":[],"errors":[
	  {"code":3,"level":"warn","type":["PartialParsing",[{}]],
	   "path":"C:\\repo\\doc\\raro.ts","message":"parcial"}]}`)
	pa := res.parciales()
	if len(pa) != 1 {
		t.Fatalf("el PartialParsing debe salir en parciales: %+v", pa)
	}
	rel := relDe(`C:\repo`, pa[0].Path)
	if rel != "doc/raro.ts" {
		t.Errorf("relDe = %q; el recibo cuelga del archivo relativo al repo con '/'", rel)
	}

	in := engines.Input{Files: []gitdiff.ChangedFile{
		{Path: "doc/raro.ts", Status: "M"},
		{Path: "doc/bien.ts", Status: "M"},
	}}
	porRuta := map[string]engines.Recibo{}
	for _, r := range recibos(in, []string{rel}) {
		porRuta[r.Unidad.Ruta] = r
	}
	if porRuta["doc/raro.ts"].Estado != engines.CoberturaParcial {
		t.Errorf("el objetivo parcial debe llevar recibo parcial: %+v", porRuta["doc/raro.ts"])
	}
	if porRuta["doc/bien.ts"].Estado != engines.CoberturaCompleta {
		t.Errorf("el objetivo intacto debe llevar recibo completo: %+v", porRuta["doc/bien.ts"])
	}
}
...
// POR ARCHIVO. Medido: el repo portal-cliente, con 854 archivos, genera 105 029
...
func TestLotesRespetanElLimiteDeLineaDeComandos(t *testing.T) {
	...
	for i := range objetivos {
		objetivos[i] = "C:\\Users\\alguien\\Desktop\\01-Proyectos-GitHub\\EMPRESA-EJEMPLO\\portal-cliente\\backend\\internal\\infra\\archivo.go"
	}
...
func (c *cacheDePrueba) Guardar(entradas []engines.Cacheable) {
	...
	for _, e := range entradas {
		if e.Vigente != nil && e.Vigente() {
			c.guardados[e.Clave] = e.Findings
		}
	}
}
...
func TestCacheTotalNoEjecutaSemgrep(t *testing.T) {
	...
	// La identidad se asigna como lo hace el pipeline: colectiva, tras servir
	// los aciertos. Aquí se ejercita ESA asignación sobre lo que salió del caché.
	finding.AsignarHuellas(fs, nil)
	for _, f := range fs {
...
func TestPorArchivoAtribuyeYRegistraLimpios(t *testing.T) {
	...
	entradas := porArchivo(fs, analizados, t.TempDir())
	if len(entradas) != 2 {
		t.Fatalf("esperaba 2 entradas (a y b; sin-huella no es cacheable): %v", entradas)
	}
