...
func TestSoloElSimboloAlcanzadoEsHallazgo(t *testing.T) {
	...
	// La identidad la asigna finding.AsignarHuellas (colectiva, huellas v2);
	// el parser garantiza el INSUMO: la identidad semántica módulo@versión.
	if f.LineContent == "" {
		t.Error("sin LineContent (módulo@versión) la huella saldría sobre vacío")
	}
}
