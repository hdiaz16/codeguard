...
func TestErroresBloqueanYNormalizanPaths(t *testing.T) {
	...
	if f.Message == "" { // W6: el parser ya no pone LineContent; la línea la lee AsignarHuellas del archivo. El insumo del parser es Message.
		// La huella la asigna finding.AsignarHuellas (colectiva, v2); el
		// parser garantiza el INSUMO del hash.
...
`)
	...
	for _, f := range fs {
		// W6 (t.128): staticcheck es de la clase «mensaje»; el parser ya no pone
		// LineContent (la línea real la lee AsignarHuellas del archivo). El insumo
		// que el parser garantiza para la huella es el Message.
		if f.Severity != finding.Error || !f.Blocking || !f.Verified || f.Message == "" {
			t.Errorf("cada hallazgo debe ser error bloqueante, verificado y con insumo de huella: %+v", f)
		}
	}
}
