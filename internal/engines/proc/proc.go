...
func Correr(ctx context.Context, c *exec.Cmd, tope int64) (Salida, error) {
	...
	rep := Contencion{}
	rep.TokenRestringido, rep.Detalle = prepararSandbox(c)
	...
	// la contención es defensa en profundidad, no la frontera (W4, t.116).
	// Lo que ya no puede ser es MUDO: hasta 2026-08-23 este error se
	// descartaba sin log ni rastro, y con el job se perdía también el
	// matarile del árbol por plazo sin que nadie lo supiera.
	if cerrar, uiOK, err := contener(c); err == nil {
		rep.Job, rep.MatarileArbol, rep.LimitesUI = true, true, uiOK
		if !uiOK && rep.Detalle == "" {
			rep.Detalle = "las restricciones de interfaz no se aplicaron"
		}
		var una sync.Once
		...
		log.Printf("proc: sin job object para %s (el plazo matará solo la raíz; los nietos pueden sobrevivir): %v",
			filepath.Base(c.Path), err)
		if rep.Detalle == "" {
			rep.Detalle = "sin job object: " + err.Error()
		}
	}
	if r := recolectorDe(ctx); r != nil {
		r.Anotar(rep)
	}
