...
var permitidas = map[string]bool{
	...
	// Cadenas de herramientas. (GOFLAGS/GONOSUMDB/GONOSUMCHECK murieron en W4:
	// las dos últimas son variables muertas del toolchain y GOFLAGS es el
	// vector de envenenamiento que ambos consejeros señalaron — t.110/115.)
...
// Entorno arma el entorno con la lista COMPLETA (la unión histórica), más lo
// que se le pase. Desde W4 es el PerfilCompleto: lo usan la medición de
// contratos y EntornoGit; los motores de análisis declaran su perfil y pasan
// por EntornoDePerfil — la vía libre era el agujero (t.115: cualquier
// adaptador reintroducía lo que quisiera).
func Entorno(extra ...string) []string {
	return entornoFiltrado(permitidas, extra)
}

// entornoFiltrado es la mecánica única del filtrado: solo lo permitido, más
// los extra en la forma "CLAVE=valor", que siempre ganan.
func entornoFiltrado(permitido map[string]bool, extra []string) []string {
	out := make([]string, 0, len(permitido)+len(extra))
	...
	for _, e := range os.Environ() {
		...
		if permitido[strings.ToUpper(clave)] {
			out = append(out, e)
		}
	}
