...
var requisitos = map[string]string{
	...
	// psscriptanalyzer NO se comprueba con un binario suelto, y es decisión, no
	// olvido: el motor corre con pwsh O con powershell (5.1), y aunque el
	// ejecutable esté, hace falta que el MÓDULO PSScriptAnalyzer esté instalado
	// —un LookPath no lo prueba—. Comprobar "pwsh" mentiría hacia el lado caro
	// (diría «disponible» sin el módulo). Como tsc/eslint: hoy no se dice; el
	// panel no promete que pueda correr, pero tampoco avisa de que no. El error
	// de Run sí guía a `Install-Module PSScriptAnalyzer` cuando falta.
	"psscriptanalyzer": "",
...
var comoInstalar = map[string]string{
	...
	"actionlint":  "instala actionlint: go install github.com/rhysd/actionlint/cmd/actionlint@latest",
	"shellcheck":  "instala shellcheck: winget install koalaman.shellcheck (o el gestor de tu SO)",
}
