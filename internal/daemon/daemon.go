...
func (s *Server) handle(ctx context.Context, conn net.Conn) {
	...
	if req.Command != "" {
		...
		switch req.Command {
		case "ping":
			// El ping es del SERVIDOR, no de la UI: hasta aquí se ack'eaba por
			// accidente en la rama de comandos y encima llegaba a la UI como
			// «comando desconocido» en el log. Responde la versión del agente
			// para que doctor y el hook puedan comparar sin adivinar.
			_ = ipc.WriteResponse(conn, &ipc.Response{RunID: req.RunID, Verdict: "ok", Reason: "codeguard-daemon " + Version})
		case "apagar", "open-graph", "secreto-bloqueado":
			if s.OnCommand != nil {
				s.OnCommand(req.Command, req)
			}
			_ = ipc.WriteResponse(conn, &ipc.Response{RunID: req.RunID, Verdict: "ok"}) // ack; si el pipe se cerró, no hay nada que hacer
		default:
			// LO DESCONOCIDO NO SE ACK'EA COMO ÉXITO ([13] del plan, medido:
			// un cliente más nuevo mandando un comando que este daemon no
			// conoce recibía "ok" — la mentira de protocolo perfecta). Los
			// clientes de HOY solo miran el éxito del transporte, así que
			// responder un error estructurado no rompe a nadie; los de mañana
			// ya tienen qué leer.
			log.Println("comando desconocido por el cable:", req.Command)
			_ = ipc.WriteResponse(conn, &ipc.Response{RunID: req.RunID, Verdict: "error",
				Reason: "comando desconocido para este daemon: " + req.Command + " — actualiza el binario que quedó atrás (daemon " + Version + ")"})
		}
		return
	}
	// La negociación de protocolo ANTES de analizar ([13], turno 92): si no
	// hay intersección entre lo que el hook habla y lo que este daemon habla,
	// se rechaza ESTRUCTURADO y el hook analiza local — exactamente un run, el
	// honesto.
	if ok, min, max := protocoloCompatible(req); !ok {
		log.Printf("protocolo incompatible: el hook habla [%d,%d] y este daemon [%d,%d]", min, max, ipc.ProtocolMin, ipc.ProtocolMax)
		_ = conn.SetWriteDeadline(time.Now().Add(5 * time.Second))
		_ = ipc.WriteResponse(conn, &ipc.Response{RunID: req.RunID, Verdict: "error",
			Reason: fmt.Sprintf("protocolo incompatible: tu binario habla [%d,%d] y este daemon [%d,%d] — actualiza el que quedó atrás",
				min, max, ipc.ProtocolMin, ipc.ProtocolMax)})
		return
	}
	if s.OnRequest != nil {
		s.OnRequest(req)
...
func (s *Server) Analyze(ctx context.Context, req *ipc.Request) *ipc.Response {
	...
	if err != nil || cfg == nil {
		...
		// tipada viaja en Outcome: config ilegible es un FALLO (nadie decidió
		// saltarse nada), repo no enrolado sí es una decisión. Antes ambos
		// eran "skipped" y no había forma de distinguir «arregla tu YAML» de
		// «este repo no juega».
		resp.Verdict = "skipped"
		if err != nil {
			resp.Reason = "no se pudo leer .codeguard/config.yaml: " + err.Error()
			resp.Outcome = ipc.DeOutcome(pipeline.Finalizar(nil, pipeline.FalloConfig, err))
		} else {
			resp.Reason = pipeline.MotivoNoEnrolado
			resp.Outcome = ipc.DeOutcome(pipeline.Finalizar(
				&pipeline.Result{Verdict: pipeline.Skipped, Reason: resp.Reason}, "", nil))
		}
	...
	if rulepackID.Source == rulepack.SourceVendored && rulepackErr == nil {
		...
		log.Printf("rulepack del repo en %s: %s", filepath.Base(req.RepoRoot), rulepackID.Path)
	}
	switch {
	case errors.Is(rulepackErr, rulepack.ErrNoEncontrado):
		resp.CIParity = false
		...
		instaladas := rulepack.RulepacksInstalados(req.RepoRoot)
		if len(instaladas) == 0 {
		...
		log.Printf("paridad rota en %s: el repo pinnea el rulepack %q y no está en %s (instaladas: %s)",
			filepath.Base(req.RepoRoot), cfg.Rulepack, rulepackID.Path, strings.Join(instaladas, ", "))
	case rulepackErr != nil:
		// Presente pero inválido (symlink dentro, colisión de mayúsculas,
		// ilegible): sin digest no hay identidad que prometer. Hoy se dice y
		// el análisis sigue (observabilidad primero); la tanda (c) lo vuelve
		// rechazo cuando el manifiesto firmado exista.
		resp.CIParity = false
		resp.ParityReason = fmt.Sprintf("el rulepack %s no tiene identidad verificable: %v", cfg.Rulepack, rulepackErr)
		log.Printf("rulepack sin identidad en %s: %v", filepath.Base(req.RepoRoot), rulepackErr)
	}

	resp.Rulepack = &rulepackID
	...
	if s.Shadow != nil && s.Shadow.Store != nil {
		...
		demoted, errDegradadas = s.Shadow.Store.DemotedRules(req.RepoID, store.MinVotosDemocion, store.MaxTasaFPDemocion)
	...
	if err != nil {
		...
		// para el hook viejo; el Outcome dice la verdad que ese "skipped"
		// disfrazaba: el análisis FALLÓ — nadie decidió no correr.
		resp.Verdict = "skipped"
		resp.Reason = "el análisis no corrió: " + err.Error()
		resp.Degraded = append(resp.Degraded, fmt.Sprintf("pipeline:%v", err))
		resp.Outcome = ipc.DeOutcome(pipeline.Finalizar(nil, pipeline.FalloPipeline, err))
		return resp
	}
	// Las degradaciones que ESTE proceso anotó antes de correr el pipeline
	// (demoted:unavailable) se funden en el Result ANTES de derivar: el outcome
	// se deriva del cuadro completo, no del pedazo que vio pipeline.Run.
	res.Degraded = append(append([]string{}, resp.Degraded...), res.Degraded...)
	...
	// EL veredicto, derivado UNA vez y aquí: el hook y el panel leen esto.
	resp.Outcome = ipc.DeOutcome(pipeline.Finalizar(res, "", nil))
...
// Request, y devuelve el rango NORMALIZADO del emisor para el diagnóstico.
// La ausencia del rango (cliente anterior al handshake) se interpreta como
// legacy EXACTO {v,v} — jamás como compatibilidad universal (turno 92): un
// campo que falta no es un permiso, es una versión vieja.
func protocoloCompatible(req *ipc.Request) (bool, int, int) {
	min, max := req.ProtocolMin, req.ProtocolMax
	if min == 0 && max == 0 {
	...
	return max >= ipc.ProtocolMin && min <= ipc.ProtocolMax, min, max
}
