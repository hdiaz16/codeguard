...
func TestLosRequisitosSonLosEjecutablesQueLosMotoresInvocan(t *testing.T) {
	quiero := map[string]string{
		...
		"actionlint":         "actionlint", // actionlint.go: bin = "actionlint"
		"shellcheck":         "shellcheck", // shellcheck.go: bin = "shellcheck"
		// Vacío A PROPÓSITO, como tsc/eslint: el motor invoca pwsh O powershell
		// (5.1), y aunque el ejecutable esté, hace falta el MÓDULO
		// PSScriptAnalyzer —un LookPath no lo prueba—. Comprobar "pwsh" diría
		// «puede correr» sin el módulo. Entre callar y prometer de más, se calla.
		"psscriptanalyzer": "",
	}
