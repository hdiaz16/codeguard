...
// Los umbrales de la auto-calibración viven JUNTO al criterio y son los que
// usan tanto el daemon (al aplicar) como `codeguard stats` (al explicar).
// Antes stats re-implementaba la regla con 5 y 0.20 cableados a mano — y sin
// la exención de gitleaks que el pipeline sí aplica: podía anunciar
// «DEGRADADA a aviso» de una regla de secretos que iba a seguir bloqueando.
const (
	MinVotosDemocion  = 5
	MaxTasaFPDemocion = 0.20
)
...
func (s *Store) SaveFeedback(findingID, verdict, comment string) error {
	...
	// Antes single-Exec; ahora tx para la fila Y su evento de outbox juntos (W5).
	tx, err := s.db.Begin()
	if err != nil {
		return err
	}
	...
	if _, err = tx.Exec(`INSERT INTO feedback (id, finding_id, verdict, comment, created_at)
		VALUES (?, ?, ?, ?, ?)`, id, findingID, verdict, comment, nowISO()); err != nil {
		return err
	}
