...
func TestSoloRunsSeCorrigeAlReempujar(t *testing.T) {
	for nombre, es := range entidadesSync {
		// repos tiene su propio DO UPDATE (refresca last_seen_at): es idempotente
		// y preexistente, no la mutabilidad de riesgo que este contrato vigila.
		if nombre == "repos" {
			continue
		}
		ins := strings.Join(strings.Fields(es.ins), " ") // el SQL va alineado; el contrato no depende de los espacios
		...
		if nombre != "runs" {
			...
			if !strings.Contains(ins, "ON CONFLICT (id) DO NOTHING") {
				t.Errorf("%s perdió su ON CONFLICT (id) DO NOTHING: %s", nombre, ins)
			}
			continue
		}
		if !hayUpdate {
		...
		// El guard de revisión (W5): el DO UPDATE de runs solo aplica si la
		// revisión entrante es >= la del central — un retry viejo del insert no
		// pisa un update nuevo.
		if !strings.Contains(clausula, "sync_revision = excluded.sync_revision") {
			t.Errorf("el DO UPDATE de runs debe llevar la revisión de sync: %s", clausula)
		}
		if !strings.Contains(ins, "WHERE excluded.sync_revision >= runs.sync_revision") {
			t.Errorf("el ON CONFLICT de runs perdió el guard de revisión (un retry viejo podría pisar un update nuevo): %s", ins)
		}
	}
	for _, nombre := range []string{"repos", "runs", "findings", "feedback", "llm_calls"} {
		if _, ok := entidadesSync[nombre]; !ok {
			t.Errorf("la entidad %s desapareció del sync: el contrato de arriba dejó de cubrirla", nombre)
		}
	}
}
