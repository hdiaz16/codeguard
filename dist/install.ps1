...
if (Test-Path (Join-Path $Src "rulepacks")) {
    # Reemplazo POR VERSION, jamas fusion (la misma razon escrita en
    # build-dist): Copy-Item -Force FUSIONA, y una version ya instalada con
    # otro contenido quedaria mezclada bajo el mismo nombre — la divergencia
    # 130-vs-161 reglas medida el 2026-08-23 nacio exactamente asi. Cada
    # version que trae el paquete se borra entera y se copia entera; las
    # versiones que el paquete NO trae se conservan (last-known-good
    # implicito, docs/threat-model-rulepack.md).
    foreach ($ver in Get-ChildItem (Join-Path $Src "rulepacks") -Directory) {
        foreach ($base in @($Root, $Bin)) {
            ...
            Copy-Item $ver.FullName $carpeta -Recurse -Force
        }
    }
}
...
if ($daemonOk) {
    Ok "daemon corriendo y VERIFICADO por el pipe (el orbe vive abajo a la derecha)"
} else {
    Write-Host "  ! el daemon no confirmo en 15s: revisa con 'codeguard doctor --global'" -ForegroundColor Yellow
}
