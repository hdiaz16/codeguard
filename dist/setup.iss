...
; El reparto masivo (/SILENT y /VERYSILENT) dejaba el daemon MUERTO hasta el
; siguiente inicio de sesión: la única entrada [Run] llevaba skipifsilent y
; nada más lo arrancaba — «instalado pero inoperante» no es instalación
; (decisión de Héctor, consejo unánime). En silencioso arranca aquí y
; ArrancarYVerificarDaemon comprueba que quedó VIVO con doctor --global.
Filename: "{app}\bin\{#MyDaemonExe}"; \
    Parameters: "--tray"; \
...
// ArrancarYVerificarDaemon comprueba que el daemon quedo VIVO tras el
// arranque silencioso ([Run] con Check: WizardSilent): pregunta con
// `codeguard doctor --global` (exit 0 = healthy, con el daemon respondiendo
// al ping por el pipe) hasta 15 segundos. El resultado queda en el log de
// Inno y en %TEMP%\codeguard-daemon.estado — un reparto masivo necesita un
// rastro consultable, no un dialogo que nadie vera.
...
begin
  if not WizardSilent() then
    exit; // interactivo: lo lanza la casilla postinstall y el usuario LO VE
  Intentos := 0;
  R := -1;
  while (Intentos < 30) do
  begin
    if Exec(ExpandConstant('{app}in\codeguard.exe'), 'doctor --global', '',
  ...
  if R = 0 then
    EstadoTxt := 'daemon verificado: doctor --global healthy'
  else
    EstadoTxt := 'daemon SIN verificar tras 15s (doctor exit ' + IntToStr(R) + '): revisa con codeguard doctor --global';
  Estado(EstadoTxt);
  SaveStringToFile(ExpandConstant('{%TEMP}\codeguard-daemon.estado'), EstadoTxt + #13#10, False);
end;
...
begin
  ...
  if EsActualizacion then
    ...
    Estado('Instalando motores (gitleaks, trivy, semgrep, squawk, ruff) y compilando govulncheck y staticcheck...');
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (not WizardSilent()) and (WizardForm.FilenameLabel.Caption <> Linea) then
        begin
