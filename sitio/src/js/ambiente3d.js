/* ══════════════════════════════════════════════════════════════════════════
   El mar de nubes — v4: el remolino. (El listón lo puso mont-fort.com en
   la v3; la v4 pule la MATERIA sin tocar la identidad: curl noise en el
   flujo, octavas rotadas en el fbm y erosión del borde — y cuesta menos
   por cuadro que la v3, ver las cuentas junto al cap de resolución.)
   ...
   - DEFORMACIÓN DE DOMINIO POR CURL: el campo que empuja la nube es el
     rotacional de un potencial fbm — divergencia nula, como un fluido de
     verdad. La nube no se estira: SE ENROSCA en remolinos.
   - EROSIÓN DEL BORDE: un ruido fino, arrastrado por el propio remolino,
     come el contorno justo donde la densidad es media — el borde se DESHACE
     en jirones en vez de cortarse con un smoothstep.
...
/* seis octavas ROTADAS: girar el dominio entre octava y octava rompe la
   rejilla del ruido de valor — más detalle de borde sin delatar los ejes */
const mat2 ROT = mat2(0.795, -0.607, 0.607, 0.795);
float fbm(vec2 p) {
  float v = 0.0, a = 0.5;
  for (int i = 0; i < 6; i++) {
    v += ruido(p) * a;
...
/* al campo de FLUJO le bastan tres: es una corriente ancha, no un borde */
float fbm3(vec2 p) {
  float v = 0.0, a = 0.5;
  for (int i = 0; i < 3; i++) {
    v += ruido(p) * a;
...
/* CURL NOISE: el rotacional (dψ/dy, -dψ/dx) de un potencial fbm, por
   diferencias centradas. Divergencia nula = los billows GIRAN como fluido
   en vez de deslizarse. Se evalúa UNA vez por estrato y la muestra de
   sombra lo comparte: con eso el cuadro cuesta menos que el warp doble de
   la v3 aun con una octava más en el fbm. */
vec2 remolino(vec2 p, float t, float viento) {
  vec2 q = p * 0.85 + vec2(t * 0.020 * viento, -t * 0.013 * viento);
  float e = 0.12;
  float dy = fbm3(q + vec2(0.0, e)) - fbm3(q - vec2(0.0, e));
  float dx = fbm3(q + vec2(e, 0.0)) - fbm3(q - vec2(e, 0.0));
  return vec2(dy, -dx) / (2.0 * e);
}

/* densidad de un estrato: fbm empujado por el remolino */
float nube(vec2 p, vec2 giro, float t, float viento) {
  return fbm(p * 1.15 + giro * 0.55 + vec2(t * 0.035 * viento, t * 0.006));
}
...
void main() {
  ...
  /* La ráfaga del scroll casi no empuja la niebla: Héctor pidió que NO corra
     con el scroll — que pase lento y se quede. */
  float rafaga = u_vel * 0.04;
  ...
  for (int i = 0; i < 3; i++) {
    ...
    /* La banda desciende con el scroll a MENOS de la mitad que antes (era
       0.55+0.65·prof): así la niebla se queda en pantalla en vez de irse
       corriendo al bajar la página — «siempre se va» era esto. */
    float centroBanda = 0.78 - fi * 0.34 + u_scroll * (0.26 + 0.30 * prof)
    ...
    vec2 p = uv * (1.35 + fi * 0.85)
           + vec2(t * (0.008 + 0.012 * fi) * viento + rafaga * prof,
                  -u_scroll * (0.18 + 0.26 * prof));

    vec2 giro = remolino(p, t, viento);
    float d = nube(p, giro, t, viento);
    ...
    float dens = smoothstep(cobertura, cobertura + 0.26, d);

    /* jirones: un ruido fino, arrastrado por el remolino, come el contorno
       justo donde la densidad es media (dens*(1-dens) pica en el borde) —
       la nube se DISUELVE contra el cielo en vez de recortarse */
    float fino = ruido(p * 5.2 + giro * 1.4 + vec2(t * 0.055 * viento, -t * 0.02));
    dens = clamp(dens - dens * (1.0 - dens) * fino * 1.1, 0.0, 1.0) * banda * claro;
    if (dens <= 0.001) continue;

    /* autosombra: densidad un paso hacia la luna; si la nube se tapa a sí
       misma, ese punto está en su lado oscuro. Comparte el giro del punto
       base (el paso es corto) — la ganancia sube a 2.6 para compensar el
       gradiente que el giro congelado ya no aporta. */
    float dl = nube(p + haciaLuna * 0.14, giro, t, viento);
    float sombra = clamp((dl - d) * 2.6, 0.0, 1.0);
    ...
    /* una gota de menta en el lining — el mismo verde de la niebla del
       sitio (body::before), para que nube y fondo sean UNA atmósfera */
    plataCol = mix(plataCol, vec3(0.62, 0.76, 0.73), 0.15);

    vec3 col = mix(sombraCol, lunarCol, luz * (0.55 + 0.25 * prof)) + plataCol * plata;
    /* los jirones finos tiran a azul frío: donde la nube casi no es, su
       color se acerca al de la bruma del fondo y la disolución no se ve */
    col *= mix(vec3(0.90, 0.96, 1.07), vec3(1.0), smoothstep(0.0, 0.65, dens));
  ...
  color += grano * alfa;
  /* el mismo grano tiembla también el velo: rompe el banding que un
     degradado tan oscuro dibuja contra el negro del fondo */
  alfa = clamp(alfa + grano * 0.35 * alfa, 0.0, 1.0);
...
export function montarAmbiente({ reducido = false } = {}) {
  ...
  /* Las nubes son masas suaves: se pintan a ~0.62 de la resolución física y
     el CSS estira — fbm con curl por píxel no es gratis, y a esta escala
     nadie distingue la diferencia en un degradado. (Cuentas del cuadro: el
     remolino se evalúa una vez por estrato y la sombra lo comparte — 25
     evaluaciones de ruido por estrato contra las 30 del warp doble de la
     v3, con una octava más de detalle.) */
  ...
  function cuadro(ahora) {
    ...
    /* Suavizado más lento (0.06) y tope más bajo (±0.6): un scroll brusco ya
       no dispara la niebla; la ráfaga sube y baja despacio. */
    rafaga += ((y - scrollPrevio) / window.innerHeight - rafaga) * 0.06;
    scrollPrevio = y;

    const scroll = climaFijo != null ? climaFijo : y / alto;
    pintar((ahora - inicio) / 1000, scroll, Math.max(-0.6, Math.min(0.6, rafaga)));
  ...
  if (reducido) {
    /* Reduced motion: UN cuadro, quieto — pero nítido también tras un
       resize; si no, el CSS estira el cuadro viejo y la calma sale borrosa. */
    const unCuadro = () => pintar(0, climaFijo != null ? climaFijo : 0, 0);
    unCuadro();
    window.addEventListener("resize", unCuadro, { passive: true });
    return lienzo;
  }
